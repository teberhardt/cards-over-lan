...
{
    public sealed class WebappModule : NancyModule
    {
        public WebappModule() : base("")
        {
            Get("/gameinfo",
                p => Response.AsText(
                    JsonConvert.SerializeObject(GameManager.Instance.GetGameInfoObject(), Formatting.None),
                    "application/json"));
            Get("/", p =>
            {
                if (!string.IsNullOrWhiteSpace(Request.Headers.Referrer))
                    AnalyticsManager.Instance.RecordReferrer(Request.Headers.Referrer.Trim());
